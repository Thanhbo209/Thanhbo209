<!--
Profile README for GitHub username: Thanhbo209
Place this file at: https://github.com/Thanhbo209/Thanhbo209/blob/main/README.md
-->

<div align="center">

# Hi, I'm Pham Viet Thanh 👋

### Software Engineering Student · Backend AI Engineering Intern · Full-Stack Builder

I build backend-heavy products, AI-integrated workflows, and practical systems that turn messy real-world input into useful software.

[![Portfolio](https://img.shields.io/badge/Portfolio-thanhpham.vercel.app-0A66C2?style=for-the-badge&logo=vercel&logoColor=white)](https://thanhpham.vercel.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Pham%20Thanh-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/phmthanh/)
[![Email](https://img.shields.io/badge/Email-Contact%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:thanhagar123@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-@Thanhbo209-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Thanhbo209)

![Profile Views](https://komarev.com/ghpvc/?username=Thanhbo209&style=flat-square&color=blueviolet)

</div>

---

## About Me

I am a Software Engineering student focused on the intersection of **backend engineering, full-stack product development, and AI-powered systems**.

Right now, I am building toward a long-term path as a **SWE + AI Engineer**: someone who can design APIs, model databases, ship user-facing products, and integrate AI pipelines responsibly instead of treating AI as magic.

- 🎓 Bachelor of Software Engineering student
- 💼 Backend AI Engineering Intern at **FlyRank AI**
- 🧠 Building RAG pipelines, agents, REST APIs, data workflows, and AI-assisted backend systems
- 🛠️ Comfortable across backend APIs, database design, React UIs, Docker, and CI/CD
- 🔬 Interested in AI engineering, retrieval systems, computer vision, automation, and applied ML tooling
- 🌏 Based in Viet Nam

---

## Tech Stack

### Languages

<p>
  <img src="https://skillicons.dev/icons?i=python,ts,js,java,html,css&perline=8" />
</p>

### Frontend

<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,vite&perline=8" />
</p>

### Backend

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,spring,fastapi&perline=8" />
</p>

### Database & ORM

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mongodb,prisma&perline=8" />
</p>

### Tools & Workflow

<p>
  <img src="https://skillicons.dev/icons?i=git,github,docker,githubactions,postman,vscode&perline=8" />
</p>

### AI / Data / Computer Vision

<p>
  <img src="https://img.shields.io/badge/RAG-111827?style=for-the-badge&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/AI%20Agents-111827?style=for-the-badge&logo=googlegemini&logoColor=white" />
  <img src="https://img.shields.io/badge/YOLOv8-111827?style=for-the-badge&logo=ultralytics&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenCV-111827?style=for-the-badge&logo=opencv&logoColor=white" />
  <img src="https://img.shields.io/badge/Pandas-111827?style=for-the-badge&logo=pandas&logoColor=white" />
  <img src="https://img.shields.io/badge/NumPy-111827?style=for-the-badge&logo=numpy&logoColor=white" />
  <img src="https://img.shields.io/badge/CVAT-111827?style=for-the-badge&logoColor=white" />
</p>

---

## Featured Projects

| Project | What it does | Stack |
|---|---|---|
| [**Document AI Agents**](https://github.com/Thanhbo209/document-ai-agents) | Enterprise-leaning document AI agent platform focused on serious technical workflows. | Python |
| [**Retail Product Detection**](https://github.com/Thanhbo209/Retail-Product-Detection) | YOLOv8 retail shelf product detection pipeline with annotation conversion, CVAT review workflow, training, inference, and product counting. | Python, YOLOv8, CVAT, OpenCV |
| [**Smart Restaurant Analytics**](https://github.com/Thanhbo209/smart-restaurant-analytics) | Spring Boot analytics system with event-driven business insights and AI-powered natural language to SQL. | Java, Spring Boot |
| [**FinAI Assistant**](https://github.com/Thanhbo209/finAI-assistant) | AI-powered financial operating system with natural-language transaction parsing and hybrid backend resolution. | TypeScript, Node.js, Express, PostgreSQL, Prisma, FastAPI, React, Docker |
| [**AI Resume Analyzer**](https://github.com/Thanhbo209/ai-powered-resume-tracking) | Resume scoring and AI-assisted feedback system with request caching and session-based UX. | JavaScript, React Router, Tailwind, Zustand |
| [**AI Engineer Logs**](https://github.com/Thanhbo209/ai-engineer-logs) | Public learning log for AI engineering practice, experiments, and technical growth. | Python |

---

## What I Am Building Toward

<table>
  <tr>
    <td><strong>Backend Engineering</strong></td>
    <td>REST APIs, service layers, authentication, database modeling, validation, testing, CI/CD.</td>
  </tr>
  <tr>
    <td><strong>AI Engineering</strong></td>
    <td>RAG pipelines, agents, embeddings, document processing, evaluation, and AI-assisted workflows.</td>
  </tr>
  <tr>
    <td><strong>Full-Stack Product Work</strong></td>
    <td>React/Next.js interfaces connected to real backend systems, not just static UI demos.</td>
  </tr>
  <tr>
    <td><strong>Computer Vision</strong></td>
    <td>Dataset preparation, annotation QA, YOLO training, inference, confidence thresholds, and model iteration.</td>
  </tr>
</table>

---

## Engineering Scorecard

I avoid fake "100% skill bars." This is a more honest snapshot of where I am currently putting effort.

| Area | Current confidence | Evidence |
|---|---:|---|
| Backend APIs | Strong | Node.js/Express, Spring Boot, FastAPI, JWT, REST APIs |
| Full-stack integration | Solid | React, Next.js, Tailwind, API clients, deployment workflows |
| Database design | Solid | PostgreSQL, MongoDB, Prisma, Mongoose, JPA/Hibernate |
| AI-integrated products | Growing fast | RAG, agents, embeddings fallback, AI resume tooling |
| Computer vision pipelines | Practical hands-on | YOLOv8, CVAT, OpenCV, annotation workflows |
| Testing / CI / DevOps | Improving | GitHub Actions, Docker, Postman, CI/CD |
| System design | Active growth area | Designing larger systems, background jobs, async workflows, production reliability |

---

## GitHub Statistics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Thanhbo209&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" />
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Thanhbo209&layout=compact&theme=tokyonight&hide_border=true" />

<br />

<img src="https://streak-stats.demolab.com?user=Thanhbo209&theme=tokyonight&hide_border=true" />

<br />

<img src="https://github-profile-trophy.vercel.app/?username=Thanhbo209&theme=tokyonight&no-frame=true&row=1&column=6" />

</div>

<details>
<summary><strong>More about how I work</strong></summary>

### My engineering habits

- I use AI tools, but I verify output through source code, tests, logs, and documentation.
- I prefer building small vertical slices instead of pretending architecture is done on paper.
- I care about clean APIs, observable behavior, reproducible setup, and readable README files.
- I try to turn every project into proof: screenshots, metrics, commands, examples, and failure cases.

### Current learning focus

- Backend AI systems
- RAG and document pipelines
- Agent workflows
- Evaluation and telemetry
- Database performance
- System design
- Testing discipline

</details>

---

## Contact

<div align="center">

<a href="https://thanhpham.vercel.app">
  <img src="https://img.shields.io/badge/Portfolio-thanhpham.vercel.app-0A66C2?style=for-the-badge&logo=vercel&logoColor=white" />
</a>
<a href="https://www.linkedin.com/in/phmthanh/">
  <img src="https://img.shields.io/badge/LinkedIn-Pham%20Thanh-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<a href="https://github.com/Thanhbo209">
  <img src="https://img.shields.io/badge/GitHub-@Thanhbo209-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="mailto:thanhagar123@gmail.com">
  <img src="https://img.shields.io/badge/Email-thanhagar123%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

</div>

---

<div align="center">

### Build real things. Measure them. Break them. Improve them.

</div>
